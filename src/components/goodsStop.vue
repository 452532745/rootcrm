  ...
  <div>
    <div class="goods_box">
      ...
      >
        ...
        <el-table-column label="计量单位:" prop="goods_unit" sortable></el-table-column>
          ...
          <el-tabs v-model="activeName" @tab-click="handleClick">
            <el-tab-pane label="商品详情" name="1">
              ...
              <el-form-item label="商品类型 :" prop="type_id">
                <el-select v-model="goodsForm.goods_type_id" placeholder="请选择">
                  <el-option
...
export default {
  ...
  methods: {
    // 获取数据
    getGoods() {
      this.handleGet("/goodstop").then(datas => {
    ...
    addGoods(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.$http.post("/goodstop/add", registData).then(
            res => {
              this.$message({
                message: "添加成功",
                type: "success",
    ...
    deleteGoods(index, data) {
      ...
      this.$http.post("/goodstop/delete", deleteNode).then(
        res => {
          this.$message({
            message: "删除成功",
            type: "success",
